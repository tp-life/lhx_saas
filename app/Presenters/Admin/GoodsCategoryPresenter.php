...
{
	public function goodsCategoryNestable($menus)
	{   //dd($menus);
		if ($menus) {
			$item = '';
			foreach ($menus as $v) {
	...
	{
		if ($menu['child']) {
			return $this->getHandleList($menu['id'],$menu['class_name'],$menu['class'],$menu['child']);
		}
		$labelInfo = $menu['p_id'] == 0 ?  'label-info':'label-warning';
		return <<<Eof
				<li class="dd-item dd3-item" data-id="{$menu['id']}">
                    <div class="dd-handle dd3-handle">Drag</div>
                    <div class="dd3-content"><span class="label {$labelInfo}"><i class="{$menu['class']}"></i></span> {$menu['class_name']} {$this->getActionButtons($menu['id'])}</div>
                </li>
Eof;
	}
	/**
...
Eof;
	...
	protected function getActionButtons($id)
	{
		...
		if (auth()->user()->can(config('admin.permissions.menu.show'))) {
			$action .= '<a href="javascript:;" class="btn btn-xs tooltips showInfo" data-href="'.url('admin/goods_category',[$id]).'" data-toggle="tooltip" data-original-title="'.trans('admin/action.actionButton.show').'"  data-placement="top"><i class="fa fa-eye"></i></a>';
		}
		if (auth()->user()->can(config('admin.permissions.menu.edit'))) {
			$action .= '<a href="javascript:;" data-href="'.url('admin/goods_category/'.$id.'/edit').'" class="btn btn-xs tooltips editMenu" data-toggle="tooltip"data-original-title="'.trans('admin/action.actionButton.edit').'"  data-placement="top"><i class="fa fa-edit"></i></a>';
		}
		if (auth()->user()->can(config('admin.permissions.menu.destroy'))) {
			$action .= '<a href="javascript:;" class="btn btn-xs tooltips destroy_item" data-id="'.$id.'" data-original-title="'.trans('admin/action.actionButton.destroy').'"data-toggle="tooltip"  data-placement="top"><i class="fa fa-trash"></i><form action="'.url('admin/goods_category',[$id]).'" method="POST" style="display:none"><input type="hidden"name="_method" value="delete"><input type="hidden" name="_token" value="'.csrf_token().'"></form></a>';
		}
		$action .= '</div>';
		return $action;
	}
	...
	public function canCreateCategory()
	{
		...
		$title = $canCreateMenu ?  trans('admin/goods.welcome'):trans('admin/goods.sorry');
		$desc = $canCreateMenu ? trans('admin/goods.description'):trans('admin/goods.description_sorry');
		$createButton = $canCreateMenu ? '<br><a href="javascript:;" class="btn btn-primary m-t create_menu">'.trans('admin/goods.action.create').'</a>':'';
		return <<<Eof
...
Eof;
	...
	{
		$html = '<option value="0">'.trans('admin/goods.top_category').'</option>';
		if ($menus) {
			foreach ($menus as $v) {
				$html .= '<option value="'.$v['id'].'" '.$this->checkMenu($v['id'],$pid).'>'.$v['class_name'].'</option>';
			}
		}
		return $html;
	}
