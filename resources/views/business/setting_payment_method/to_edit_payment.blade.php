...
<div class="modal-header">
    ...
    @if($info->payment_type == 3)
        <h5 class="modal-title">编辑支付宝</h5>
    @else
        <h5 class="modal-title">编辑微信</h5>
    @endif

</div>
<div class="modal-body">
    <form class="form-horizontal" id="unit_from" method="post" action='{{url('business/setting_payment_method/to_edit_payment')}}'>
        {{csrf_field()}}
        <div class="hr-line-dashed"></div>
        <div class="form-group">
            <label class="col-sm-3 control-label">账户名称：</label>
            <div class="col-sm-5">
                <input type="text" class="form-control" name='account_name' value="{{$info->account_name}}" placeholder="账户名称" required>
            </div>
        </div>
        <div class="hr-line-dashed"></div>
        <div class="form-group">
            @if($info->payment_type == 3)
                ...
                <div class="col-sm-5">
                    <input type="text" class="form-control" name='account' value="{{$info->account}}"  placeholder="支付宝账号">
                </div>
            @else
                <label class="col-sm-3 control-label">微信账号：</label>
                <div class="col-sm-5">
                    <input type="text" class="form-control" name='account' value="{{$info->account}}" placeholder="微信账号">
                </div>
            @endif
        ...
        <div class="form-group">
            ...
            <div class="col-sm-5 ">
                ...
                <div id="picfileList" class="uploader-list">
                    @if($info->qr_code)
                        <span>
                            <img src="{{$info->qr_code}}" style="height: 80px;"/>
                            <input type="hidden" name="qr_code" value="{{$info->qr_code}}"/>
                        </span>
                    @endif
                </div>
            </div>
