    ...
    <div class="row wrapper border-bottom white-bg page-heading">
        ...
        <div class="col-lg-2">
            <div class="title-action">
                <a href="{{url('business/brand/create')}}" class="btn btn-info">添加品牌</a>
            </div>
        </div>
        @endpermission
    </div>
...
@section('js')
    ...
    <script src="{{asset('business/js/brand/brand-datatable.js')}}"></script>
    <script type="text/javascript">
        ...
        function disableStatus(status,id){
            $.post('{{url('business/brand/change')}}',{
                '_token':'{{csrf_token()}}',
                id:id,
                status:status
            },function (data) {
        ...
        function deleted(id) {
            layer.confirm('{{trans('admin/alert.deleteTitle')}}', {
                ...
                $.post('{{url('business/brand/destroy')}}',{
                    '_token':'{{csrf_token()}}',
                    id:id
                },function (data) {
                    layer.close(index);
