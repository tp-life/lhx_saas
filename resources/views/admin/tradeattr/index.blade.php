    ...
    <div class="row wrapper border-bottom white-bg page-heading">
        ...
        <div class="col-lg-2">
            <div class="title-action">
                <a href="{{url('admin/tradeattr/create')}}" class="btn btn-info">添加商品属性</a>
            </div>
        </div>
        @endpermission
    </div>
...
@section('js')
    ...
    <script src="{{asset('admin/js/tradeattr/datatable.js')}}"></script>
    <script type="text/javascript">
        ...
        function disableStatus(status,id){
            $.post('{{url('admin/tradeattr/change')}}',{
                '_token':'{{csrf_token()}}',
                id:id,
                status:status
            },function (data) {
        ...
        function deleted(id) {
            layer.confirm('{{trans('admin/alert.deleteTitle')}}', {
                ...
                $.post('{{url('admin/tradeattr/destroy')}}',{
                    '_token':'{{csrf_token()}}',
                    id:id
                },function (data) {
                    layer.close(index);
